...
trait DynDummies { self: DynFluentBuilding =>
  ...
  trait DynzDummies { self: DynzFluentBuilding =>
      lazy val Dummy: Dynz = Dyns.Dummy.asIterator

      // ---------------------------------------------------------------------------
      def dummy(value : NakedValue)                    : Dynz = Dyns.dummy(value)         .asIterator
      def dummy(value1: NakedValue, value2: NakedValue): Dynz = Dyns.dummy(value1, value2).asIterator }
