...
trait IntegerLike[I] extends NumberLike[I] { // TODO: rename to Integral? use stdlib's Integral more?
    ...
    // ===========================================================================
    object IntegerLike {
      private[aptdata] class Int__  (val value: Int)   extends IntegerLike[Int]
      private[aptdata] class Long__ (val value: Long)  extends IntegerLike[Long]
      private[aptdata] class Short__(val value: Short) extends IntegerLike[Short]
      private[aptdata] class Byte__ (val value: Byte)  extends IntegerLike[Byte]

      // ---------------------------------------------------------------------------
      // order matters (see a241125101924)
      private[aptdata] implicit def _byte (value: Byte) : IntegerLike[_] = new Byte__ (value)
      private[aptdata] implicit def _short(value: Short): IntegerLike[_] = new Short__(value)
      private[aptdata] implicit def _int  (value: Int)  : IntegerLike[_] = new Int__  (value)
      private[aptdata] implicit def _long (value: Long) : IntegerLike[_] = new Long__ (value) }
