...
trait RealLike[R] extends NumberLike[R] {
    ...
    // ===========================================================================
    object RealLike {
      private[aptdata] class Double__(val value: Double) extends RealLike[Double] { val either = Left (value) }
      private[aptdata] class Float__ (val value: Float)  extends RealLike[Float]  { val either = Right(value) }

      // ---------------------------------------------------------------------------
      // order matters (see a241125101924)
      private[aptdata] implicit def _float (value: Float) : RealLike[_] = new Float__ (value)
      private[aptdata] implicit def _double(value: Double): RealLike[_] = new Double__(value) }
