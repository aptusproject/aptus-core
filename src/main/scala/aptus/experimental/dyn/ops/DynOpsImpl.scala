...
trait DynOpsImpl {
  ...
  // ---------------------------------------------------------------------------
  /** MUST have distinct keys */ def merge(that: Dyn): Dyn = dyn.data.sngl.Dyn.build((data: Iterable[Entry]) ++ (that.data: Iterable[Entry]))
...
final def replace          (key: Ren): _Replace = new _Replace(???, guaranteed = false)
  ...
  final def replaceGuaranteed(key: Key): _Replace = new _Replace(key, guaranteed = true)
    ...
    final class _Replace private[dyn] (key: Key, guaranteed: Boolean) {
