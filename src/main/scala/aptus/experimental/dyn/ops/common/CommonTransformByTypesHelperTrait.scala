...
// ---------------------------------------------------------------------------
  ...
  final def bigDec  (f: BigDecimal    => NakedValue): Data = using { v => v.bigDecOpt  .map(f).getOrElse(e.throwBasicType(deef, target, v)(_._BigDec)) }
  final def date    (f: LocalDate     => NakedValue): Data = using { v => v.dateOpt    .map(f).getOrElse(e.throwBasicType(deef, target, v)(_._Date)) }
  final def dateTime(f: LocalDateTime => NakedValue): Data = using { v => v.dateTimeOpt.map(f).getOrElse(e.throwBasicType(deef, target, v)(_._DateTime)) }
